...
class TeamTaskListTool(Tool):
    ...
    def __init__(
        ...
        super().__init__(
            name=name,
            description='List tasks from the team shared task board.',
    ...
    def run(self, parameters: Dict[str, Any]) -> str:
        ...
        try:
            tasks = self._team_manager.list_board_tasks(team_name=team_name, status=status)
            return self.create_success_response(
                data={"tasks": tasks, "total": len(tasks)},
                text=f"Listed {len(tasks)} team tasks.",
                params_input=params_input,
            ...
            return self.create_error_response(
                ...
                message=f"TeamTaskList failed: {exc}",
                params_input=params_input,
                time_ms=int((time.monotonic() - start_time) * 1000),
            )
